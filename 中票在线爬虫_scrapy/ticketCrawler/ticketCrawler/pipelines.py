...
class TicketcrawlerPipeline(object):
    ...
    def process_item(self, item, spider):
        if item.__class__==TicketCrawlerItem:
            try:
                ...
                self.createTable()
                #插入sql语句
                sqlInsert = "INSERT INTO ticketCrawler.tickets(name,price,time,address,type,url,introduction)values(%s,%s,%s,%s,%s,%s,%s)"
                # #更新sql语句
                # sqlUpdate = "UPDATE "
    ...
    def createTable(self):
        ...
        try:
            #创建连接
            engine = create_engine("mysql+mysqldb://root:bwx0605@127.0.0.1:3306/ticketCrawler?charset=utf8", max_overflow=10000)
            #获取元数据
            metadata = MetaData()
            #定义表
    ...
    def insertIntoTable(self,url,sql,item):
        try:
            ...
            #     # engine.execute(sql, (item['name'], item['price'], item['time'], item['address'], item['type'], item['url'],item['introduction']))
            # else:
            engine.execute(sql,(item['name'],item['price'],item['time'],item['address'],item['type'],item['url'],item['introduction']))
        except Exception as err:
            print err
